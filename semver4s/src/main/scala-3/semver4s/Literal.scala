...
object Literal:
  ...
  private def unliftLiteral[A: FromExpr, B: ToExpr](a: Expr[A], f: A => Option[B])(using
      Quotes
  ): Expr[B] =
  ...
  given ToExpr[Partial] with
    ...
    def apply(x: Partial)(using Quotes): Expr[Partial] = x match
      ...
      case Patch(maj, min, pat) =>
        '{ Partial.unsafe(${ Expr(maj) }, ${ Expr(min) }, ${ Expr(pat) }) }
      case Pre(maj, min, pat, pre) =>
        '{ Partial.unsafe(${ Expr(maj) }, ${ Expr(min) }, ${ Expr(pat) }, ${ Expr(pre) }) }
    ...
    sc.parts.toList match
      ...
      case _ =>
        quotes.reflect.report.error(
          "version interpolator is not allowed to have any interpolated values"
        )
        ???
  }
  ...
  private def interpolatedMatcherImpl(esc: Expr[StringContext])(using Quotes) = {
    ...
    sc.parts.toList match
      ...
      case _ =>
        quotes.reflect.report.error(
          "matcher interpolator is not allowed to have any interpolated values"
        )
        ???
  }
