/** Semver4s provides semver versions and matchers and parsers for them.
  ...
  * strings with [[semver4s.parseMatcher]] and [[semver4s.parseVersion]].
  *
  * For supported ranges, see the NPM documentation over at
  * https://www.npmjs.com/package/semver#ranges
  *
  * For literals, importing [[Literal]]. _ brings the interpolators m and v in scope, which you can
  * use to write, for example a literal version `v"1.2.3"` or literal matcher `m"~1.3.2 || 2.0.x -
  * 2.2.x"`, which are parsed and validated at compile time.
  */
