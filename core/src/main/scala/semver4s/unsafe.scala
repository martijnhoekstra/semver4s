...
object unsafe {
  ...
  /** get the version from a string, or throw an exception if it has a bad format
    */
  def version(versionString: String): Version = coreparse.SemverParser.parseVersion(versionString).fold(
  ...
  /** get a matcher from a string, or throw an exception if it has a bad format
    */
  def matcher(matcherString: String): Matcher = coreparse.MatcherParser.parseMatcher(matcherString).fold(
  ...
  implicit final class UnsafeVersionOps(val versionString: String) extends AnyVal {
    ...
    /** Checks wheter the receiver string, when interpreted as a semver version matches the given
      ...
      *
      * @example "1.2.3".matchesRange(">=1 <2") //true
      * @example "something".matchesRange(">=1 <2") //IllegalArgumentException
      */
    ...
    /** Checks wheter the receiver string, when interpreted as a semver version matches the given
      ...
      *
      * @example "1.2.3".matchesRange(">=1 <2") //true
      * @example "something".satisfies(">=1 <2") //IllegalArgumentException
      */
  ...
  implicit final class UnsafeMatcherOps(val rangeString: String) extends AnyVal {
    ...
    /** Checks whether the receiver string, when interpreted as a semver matcher, matches the argument string
      * interpreted as a semver version
      * 
      * @example ">=1 <2".matchesVersion("1.2.3")
      */
