...
object MatcherParser {
  ...
  val partial: P[Partial] = {
    ...
    star
      .as(Wild)
      .orElse(major.map {
        case (maj, (List(min, pat), Some(pre))) => Partial.unsafe(maj, min, pat, pre.toList)
  ...
  val range: P[And] = P.oneOf(
    List(
      hyphen.backtrack
        ...
        .map(h => And(List(h))),
      simple.repSep(P.char(' ').backtrack).map(nel => And(nel.toList))
    )
  )

  val logicalOr = P.string("||").surroundedBy(P.char(' ').rep)
  val rangeSet  = range.repSep(logicalOr).map(nel => Or(nel.toList))
